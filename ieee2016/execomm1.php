<!DOCTYPE html>
<html>

<head>
	<!-- Meta -->
	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no">
	<meta name="description" content="">
	<meta name="author" content="">
	<title>IEEE 2016 - Execomm</title>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<meta name="viewport" content="width=device-width, initial-scale=1.0" />
	<link rel="shortcut icon" href="assets/img/ico.png">
	<!-- Styles -->
		<link rel="stylesheet" type="text/css" href="assets/css/bootstrap.css" />
		<link rel="stylesheet" type="text/css" href="assets/css/font-awesome.css" />
		<link rel="stylesheet" type="text/css" href="assets/css/theme.css" />
		<link href='http://fonts.googleapis.com/css?family=Lato:100,300,400,700,900,100italic,300italic,400italic,700italic,900italic' rel='stylesheet' type='text/css'>
		<link rel="stylesheet" type="text/css" href="assets/css/default.css" />
		<link rel="stylesheet" type="text/css" href="assets/css/animate.css" />
		<link rel="stylesheet" type="text/css" href="assets/css/yamm.css" />
		<link rel="stylesheet" type="text/css" href="assets/css/shortcodes.css" />
	
	<!--[if lt IE 9]>
			<script src="assets/js/html5shiv.js"></script>
			<script src="assets/js/respond.min.js"></script>
			<![endif]-->
	<!-- Favicon -->
</head>
	<body data-spy="scroll" data-target=".navbar-collapse" data-offset="100"> 
	<!-- Navigation -->
 <?php 
  	$branch = "active";
  	$about = "";
	$chapters = "";
	$gallery = "";
	$faq = "";
	$contact = "";
  	include('nav.php'); 
  ?>
<section class="title-section main-color">
	<div class="container">
		<h4>Branch Information</h4>
	</div>
</section>	
<!-- section : [!RECENT WORK ITEMS!]-->
<section class="container work-isotope" >
<div class="wow bounceInDown delay-1">
	<div class="row">
		<div class="col-md-12">	
			<h2 style="font-size: 50px; font-style: bold;color: #6699CC; text-align: center;">
					
BRANCH EXECOMM 2016

			</h2>		
		</div>
		
	</div>

	<div class="content">
		<div class="container layout">
			

		
			
			<div class="row">
			

			<p style="font-size: 40px; font-style: bold;" align="center">Core Team</p><br>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Sahil.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/Sahilrulz"><i class="icon-facebook"></i></a></li>
							
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Sahil Jain
						</div>
						<div class="member-position">
							Chairperson
						</div>
					</div>
					<p align="center">+91-8128552048</p>
				</div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Khyati.jpg" height="250px" width="200px">
						<ul class="b-social">
							
							
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Khyati Mahajan
						</div>
						<div class="member-position">
							Vice-Chairperson
						</div>
					</div>
					<p align="center">+91-9727247344</p>
				</div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/raj.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/profile.php?id=100006523120386"><i class="icon-facebook"></i></a></li>
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Raj Suvariya
						</div>
						<div class="member-position">
							Treasurer
						</div>
					</div>
					<p align="center">+91-9099809050</p>
					
				</div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Abhishek.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://facebook.com/parikhabhi007"><i class="icon-facebook"></i></a></li>
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Abhishek Parikh
						</div>
						<div class="member-position">
							Secretary
						</div>
					</div>
					<p align="center">+91-9429520590</p>
				</div>




				<style type="text/css">
				 .blank{
				 	height: 300px;
				 	width: 300px;
				}
				</style>


				
				<br><br>
				<p style="font-size: 40px; font-style: bold;" align="center">SIG Embedded</p><br>
				<div class="b-member m-quad row-item col-md-3"></div>
				<div class="b-member m-quad row-item col-sm-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Harsh.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/harshthakkar456"><i class="icon-facebook"></i></a></li>
						
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Harsh Thakkar
						</div>
						<div class="member-position">
							SIG Embedded
						</div>
					</div>
					<p align="center">+91-8866821682</p>
				</div>
				<div class="b-member m-quad row-item col-sm-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Birva.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/profile.php?id=100009308644191"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Birva Patel
						</div>
						<div class="member-position">
							SIG Embedded
						</div>
					</div>
					<p align="center">+91-9033572366</p>
				</div>

				


				

				<br><br>
				<!-- NEW -->
				<div class="col-md-12">
				<p style="font-size: 40px; font-style: bold;" align="center">SIG Mobile Apps</p><br>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Akar.jpg" height="250px" width="200px" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/akar.sheth"><i class="icon-facebook"></i></a></li>
						
						</ul>
					</div>

					<div class="member-meta">
						<div class="member-name">
							Akar Sheth
						</div>
						<div class="member-position">
							SIG Mobile Apps
						</div>
					</div>
					<p align="center">+91-9724164467</p>
				</div>


				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Urvashi.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="http://www.facebook.com/urvashi.jain.33"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Urvashi Banthia
						</div>
						<div class="member-position">
							SIG Mobile Apps
						</div>
					</div>
					<p align="center">+91-8000241589</p>
				</div>
				

				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Tirth.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="http://www.facebook.com/tirth.shah.3158"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Tirth Shah
						</div>
						<div class="member-position">
							SIG Mobile Apps
						</div>
					</div>
					<p align="center">+91-9727127588</p>
				</div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Milap.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="http://www.facebook.com/milap.shah.144"><i class="icon-facebook"></i></a></li>
						
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Milap Shah
						</div>
						<div class="member-position">
							SIG Mobile Apps
						</div>
					</div>
					<p align="center">+91-8460733661</p>
				</div>


</div>


			
				<div class="col-md-12">
				<p style="font-size: 40px; font-style: bold;" align="center">WIE</p><br>
				<div class="b-member m-quad row-item col-md-1"></div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Prachi.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="http://www.facebook.com/prachi.agrawal.9659"><i class="icon-facebook"></i></a></li>
						
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Prachi Agrawal
						</div>
						<div class="member-position">
							WIE
						</div>
					</div>
					<p align="center">+91-9712015780</p>
				</div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Konica.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/konica.tallari"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Konica Tallari
						</div>
						<div class="member-position">
							WIE
						</div>
					</div>
					<p align="center">+91-7572811802</p>
				</div>

				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Parita.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/parita.patel.397"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Parita Patel
						</div>
						<div class="member-position">
							WIE
						</div>
					</div>
					<p align="center">+91-9925220141</p>
				</div>
				
				<div class="b-member m-quad row-item col-md-1"></div>



</div>












				
								<div class="col-md-12">

				<p style="font-size: 40px; font-style: bold;" align="center">Event Coordinators</p><br>
				

				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Nilay.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="http://www.facebook.com/prachi.palod.15"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Nilay Shrimali
						</div>
						<div class="member-position">
							Event Coordinator
						</div>
					</div>
					<p align="center">

+91-7046295423</p>
				</div>


				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/PrachiP.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="http://www.facebook.com/prachi.palod.15"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Prachi Palod
						</div>
						<div class="member-position">
							Event Coordinator
						</div>
					</div>
					<p align="center">

+91-9727066714</p>
				</div>


				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Nikisha.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/nikisha.patel.25"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Nikisha Patel
						</div>
						<div class="member-position">
							Event Coordinator
						</div>
					</div>
					<p align="center">

+91-9978212666</p>
				</div>


				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Divya.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="http://www.facebook.com/divya.solucky.6198"><i class="icon-facebook"></i></a></li>
						
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Divyakumar Solanki
						</div>
						<div class="member-position">
							Event Coordinator
						</div>
					</div>
					<p align="center">

+91-7202839225</p>
				</div>
		
				</div>







				<div class="col-md-12">
					<div class="b-member m-quad row-item col-md-3 blank"></div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Dhruv.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="http://www.facebook.com/dhruv.barochia"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Dhruv Bharochiya
						</div>
						<div class="member-position">
							Event Coordinator
						</div>
					</div>
					<p align="center">

+91-8866005687</p>
				</div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Kishan.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/profile.php?id=100010316677455"><i class="icon-facebook"></i></a></li>
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Kishan Patel
						</div>
						<div class="member-position">
							Event Coordinator
						</div>
					</div>
					<p align="center">
+91-9714953058</p>
				</div>
				


				</div>


			<div class="col-md-12">
				<p style="font-size: 40px; font-style: bold;" align="center">Other Team Members</p><br>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Dhaval.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/adhavalboy"><i class="icon-facebook"></i></a></li>
							
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Dhaval Prajapati
						</div>
						<div class="member-position">
							Designer
						</div>
					</div>
					<p align="center">+91-9974956266</p>
				</div>
				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Mrugesh.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank" href="http://www.facebook.com/"><i class="icon-facebook"></i></a></li>
							
							
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Mrugesh
						</div>
						<div class="member-position">
							MDO
						</div>
					</div>
					<p align="center">+91-9426391296</p>
				</div>


				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Saurav.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://facebook.com/saorav"><i class="icon-facebook"></i></a></li>
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Saurav Pratihar
						</div>
						<div class="member-position">
							Webmaster
						</div>
					</div>
					<p align="center">+91-9033399314</p>
				</div>


				<div class="b-member m-quad row-item col-md-3">
					<div class="member-photo">
						<img src="assets/img/execomm/Chirang.jpg" height="250px" width="200px">
						<ul class="b-social">
							<li><a class="fb" target="_blank"href="https://www.facebook.com/"><i class="icon-facebook"></i></a></li>
						</ul>
					</div>
					<div class="member-meta">
						<div class="member-name">
							Chirang Malaviya
						</div>
						<div class="member-position">
							Volunteer
						</div>
					</div>
					<p align="center">+91-8107058444</p>
					
				</div>
				
</div>


				
			</div>
			</div>
		</div>
	</div>
</section>





	
<?php include('footer.php'); ?>
<script type="text/javascript" src="assets/js/jquery.js"></script>
<script type="text/javascript" src="assets/js/bootstrap.js"></script> 
<script type="text/javascript" src="assets/js/jquery.countdown.min.js"></script>
<script type="text/javascript" src="assets/js/wow.min.js"></script>
<script type="text/javascript" src="assets/js/progressbars.js"></script>
<script type="text/javascript" src="assets/js/isotope.pkgd.min.js"></script>
<script type="text/javascript" src="assets/js/jquery.classyloader.js"></script>
<script type="text/javascript" src="assets/js/main.js"></script>
<script type="text/javascript" src="assets/js/main2.js"></script>
</body>

</html>
